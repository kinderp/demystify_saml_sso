...
------------ | ------------- | -------------
`/` | views.IndexView | It's sp login page. If an user is not logged in idp portal it shows a link to idp login page otherwise it shows user's page into sp portal.
`saml2/login/` | views.login | This view initiates the SAML2 Authorization handshake. It will create a SAMLRequest and redirect these data to the _idp sso entry page_ `/idp/sso/post`, from there user will be redirected to idp login process page `idp/login/process/` where idp auth process will be started
